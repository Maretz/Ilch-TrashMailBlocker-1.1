    ...
    case 2 :
        ...
        if (empty($name) OR empty($email) OR $name != $xname OR $ch_name == false OR ! $chkAntispam) {
            ...
            if (isset($_POST['register'])) {
                ...
                $mailmsg = '';  // Start Trashmail Abfrage
                $mailar = array();
                $trashlist = $allgAr['trashmail'];
                $trashlistAr = explode(',', $trashlist);
                $trashmailAr = array_unique($trashlistAr, SORT_STRING);
                $newlisttrashmail = implode(',', $trashmailAr);
                if ($newlisttrashmail) {
                    $emailhost = explode('@', $email);
                    foreach ($trashmailAr as $key => $value) {
                        if ($emailhost[1] == $value) {
                            $fehler = 'Unzul&auml;ssige Email Adresse';
                        }
                    }
                }   // Ende Trashmail Abfrage
            } else {
                $fehler = '';
            }
            $tpl = new tpl('user/regist');
